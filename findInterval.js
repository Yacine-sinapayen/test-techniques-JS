...
/* Je créer une boucle qui génère des nombres aléatoir grace entre 0 et 100 et les envoie dans mon numbersau.
...
// Résultat
// console.log('tableau non trié en entré de fonction ' + numbers);
...
Contraintes: numbers contient au moins deux éléments et au maximum 100 000 éléments. La solution qui privilégie la vitesse d'exécution pour les tableaux de grande taille obtiendra le plus de points. La différence entre deux éléments ne dépassera jamais la capacité d'un entier pour votre langage.

Comment faire ? :

1 - Je vais trier mon tableau par ordre croissant
2 -  Je vais parcourir mon tableau en calculant l'intervalle entre chaque paire de nombre consécutifs.
  2.a - Je vais récupérer :
  ...
  2.b j'instencie ma 'const interval' qui va récupérer le résultat de cette soustraction
3 - Si l'intervalle est positif et plus petit que le plus petit intervalle trouvé jusqu'a présent, je met à jour ma valeur 'smallesInterval'. Si aucun interval positif n'a été trouvé je continue d'ittérer jsuqu'a que la condition soit vrai */

function findSmallestInterval() {
  var smallesInterval = Infinity;

  // 1
  tableSorted = numbers.sort((a, b) => a - b);
